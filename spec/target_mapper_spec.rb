...
module Babelicious
  ...
  describe MapDefinition do 
    ...
    before(:each) do
      ...
      @map_definition.register_mapping(@opts)
    end
    ...
    describe "#register_from" do 
      ...
      it "should add source to mappings array" do 
        ...
        # expect
        @map_definition.mappings.should == [map_rule]
      end 
    ...
    describe "#register_include" do
      ...
      before(:each) do
        ...
        @other_map_definition = mock("MapDefinition", :direction= => nil, :register_mapping => nil, :mappings => [@other_map_rule])
        Mapper.mappings[:another_mapping] = @other_map_definition
      end
      ...
      it "should retrieve mappings from included map definition" do
        # expect
        @other_map_definition.should_receive(:mappings)
      ...
      it "should add its own mappings to those from included map definition" do
        ...
        # expect
        @map_definition.mappings.last.source.should == @dupd_source
        @map_definition.mappings.last.target.should == @dupd_target
      end
    ...
    describe "#register_mapping" do 
      ...
      describe "setting target" do 
        ...
        describe "when map is called multiple times" do 
          ...
          def do_process
            @map_definition.reset
            @map_definition.direction = {:from => :xml, :to => :hash}
            @map_definition.register_mapping({:to => "bar/foo", :from => "foo/bar"})
            @map_definition.register_mapping({:to => "bar/baz", :from => "foo/baz"})
            @map_definition.register_mapping({:to => "bar/boo", :from => "foo/boo"})
          end
      ...
      context "when :to or :from are not set" do 
        ...
        it "should raise an error" do 
          running { @map_definition.register_mapping({})}.should raise_error(MapDefinitionError)
        end
    ...
    describe "#register_to" do 
      ...
      it "should add source to mappings array" do 
        ...
        @map_definition.reset
        @map_definition.mappings << map_rule
        @map_definition.register_to
    ...
    describe "#reset" do 
      ...
      it "should set mappings and direction to nil" do 
        ...
        # expect
        @map_definition.mappings.should be_empty
